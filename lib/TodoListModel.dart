...
class TodoListModel extends ChangeNotifier {
  ...
  getTodos() async {
    //コントラクト内の関数の使い方はclient.call(コントラクト,関数,パラメータ)
    //タスクのカウントを取得．なぜかリスト形式
    ...
    for (var i = 0; i < totalTask.toInt(); i++) {
      //BigInt.fromは指定された数値から大きな整数を作成する
      //paramsは多分関数の引数を指定できるっぽい
      //callは必ずリストで返すのか？
      //指定した番号のタスクが入る．リストにindex,taskName,isCompleteの順で入る．
      var temp = await _client!.call(
          contract: _contract!, function: _todos!, params: [BigInt.from(i)]);
      //とりあえずタスク名が空じゃないかどうかを確認している
      if (temp[1] != "")
        //タスクを追加
        todos.add(
          Task(
  ...
  addTask(String taskNameData) async {
    ...
    notifyListeners();
    //更新したことをブロックチェーンに教える
    //client.sendTransaction(秘密鍵から生成されたcredentials,Transaction.callContract())
    //Transaction.callContract(コントラクト,関数,パラメータ)
